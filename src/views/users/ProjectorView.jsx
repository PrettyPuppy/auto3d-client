...
const ProjectorView = () => {
  ...
  const handleFileChange = async (e, id) => {
    ...
    const res = await fetch(`${import.meta.env.VITE_API_ROOT}/v1/project/uploadProjection/${id}`, {
      method: 'post',
      body: formData
    })
      ...
      <div className='bg-[#050505] w-[100vw] h-[100vh] text-[#E6D9C6]' style={{ fontFamily: 'Copperplate Gothic Bold' }}>
        ...
        <div className="px-28 pt-20 max-h-[90vh] overflow-hidden">
          <table className="table h-[50vh]">
            ...
            <tbody>
              {isLoad &&
                projects.filter(item => item.is_project).map((project, idx) => (
                  <tr key={idx}>
                    <th>{idx + 1}</th>
                    <td>
                    ...
                    <td>
                      {project.projection_is_accepted
                        ? <p>Accepted!</p>
                        : <a href={project.fbxUrl} onClick={() => handleAccept(idx, project.id)} className='underline cursor-pointer hover:text-white'>Model</a>
                      }
                    </td>
                    <td>
                      <input ref={(ele) => fileRef.current[idx] = ele} type='file' className='hidden' accept='.gltf' onChange={(e) => handleFileChange(e, project.id)} />
                      <button className='btn btn-circle' disabled={ !project.projection_is_accepted | project.projection_status} onClick={() => handleFileUpload(idx)}>
        ...
        <Link to={'/modeller'}>
          <p className='absolute left-5 top-6 text-xs'>Modeller</p>
        </Link>
      </div>
    </>
  )
